...
async function assertNoUiBreakage(page, label, failures) {
  ...
  if (info.surface === "operator" && info.innerWidth <= 760) {
    ...
    if (info.operatorMobileDockButtonHeights.length) {
      ...
      if (minDockButtonHeight < 40 || maxDockButtonHeight > 40) failures.push(`${label}: operator mobile dock buttons must be 40px ${JSON.stringify(info.operatorMobileDockButtonHeights)}`);
    }

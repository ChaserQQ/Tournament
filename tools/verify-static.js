...
const surfaceQa = readText("tools/qa-surface-check.cjs");
...
if (!buildJs.includes("version: 289") || !appJs.includes("RTDB_WRITE_PROTOCOL_V279 = 279") || !indexHtml.includes("build.js?v=289") || !indexHtml.includes("app.js?v=289") || !indexHtml.includes("app.css?v=284") || !indexHtml.includes("operator-mobile.css?v=285")) fail("v289 build/app assets, unchanged v284 shared CSS and v285 operator-mobile CSS assets, and v279 server-enforced write protocol markers must stay aligned.");
if (!appJs.includes("calculateGroupCountV280") || !appJs.includes("ensureAdvanceStageCompleteV280") || !appJs.includes("ensurePointStageCompleteV280") || !appJs.includes("winnersByGroup.some(players => players.length !== 1)")) fail("v280 bracket planning and qualifier advancement guards are incomplete.");
if (!matchSimulationQa.includes("incomplete groups advanced and dropped racers") || !matchSimulationQa.includes("points3 tied-cutoff playoff did not converge") || !matchSimulationQa.includes("crow semi accepted three qualifiers without one winner per group")) fail("tools/qa-match-simulation.cjs must guard the v280 bracket advancement regressions.");
if (!appJs.includes("serverOperationLeaseIsAuthoritativeV281") || !appJs.includes("retireLegacyOperationLockV281") || !appJs.includes('authority: "server-lease-v281"') || !operatorQa.includes("operation-lease-single-authority-v281") || !operatorQa.includes("stale legacy operation lock blocked the authoritative server lease")) fail("v281 authenticated operation ownership must use one authoritative server lease and retire stale local locks.");
if (!appJs.includes("operationLeaseUiStateV282") || !appJs.includes('"auto-enter-v282"') || !operatorQa.includes("draft-auto-operation-lease-v282")) fail("v282 authenticated draft operators must auto-acquire and accurately display the venue lease.");
if (!appJs.includes("live-empty-slots-v283") || !appJs.includes("data-v283-total-slots") || !appJs.includes('label: "뒤로가기", onClick: "goBack()"') || !appJs.includes('label: "기록", onClick: "openDashboardPage()"') || !appJs.includes("live-lobby-usability-v283")) fail("v283 LIVE lobby compaction and navigation markers are incomplete.");
if (!appCss.includes("v283: empty LIVE slots") || !surfaceQa.includes("compactEmptySlots")) fail("v283 compact LIVE lobby contract must stay covered by CSS and QA.");
if (!appCss.includes("v284: compact mobile navigation") || !operatorMobileCss.includes("--op-mobile-button-v284: 40px") || !operatorMobileCss.includes("--op-mobile-field-v284: 44px") || !surfaceQa.includes("mobile live navigation must share one row") || !operatorQa.includes("operator mobile dock buttons must be 40px")) fail("v284 mobile button hierarchy and compact navigation contracts must stay covered by CSS and QA.");
if (!appJs.includes("resolveOperationTakeoverPreflightV285") || !appJs.includes("stale-local-operation-recovery-v285") || !appJs.includes("operator-group-collapsible-v285") || !operatorMobileCss.includes("v285: the current mobile bracket group stays open") || !operatorQa.includes("stale-local-takeover-v285") || !operatorQa.includes("operatorBracketDensityV285")) fail("v285 stale-local lease recovery and current-group mobile bracket compaction must stay covered by runtime, CSS, and QA.");
if (!appJs.includes('serverOffsetRef.once("value")') || appJs.includes('db.ref(".info/serverTimeOffset").get()') || !appJs.includes("operationLeaseStartFailureMessageV286") || !operatorQa.includes("Invalid token in path") || !operatorQa.includes("final shortcut visible before final decision") || !operatorQa.includes("final-shortcut-gate-v286")) fail("v286 Firebase /.info listener lease recovery, accurate start failure copy, and final-action visibility gate must stay covered by runtime and QA.");
if (!appJs.includes("runReservedLeaseTransactionV287") || !appJs.includes("current == null ? reservedLease : current") || !operatorQa.includes("__qaFirebaseNullFirstTransactionPathsV287") || !operatorQa.includes("nullFirstTransactionCount < 2") || !databaseRulesQa.includes("draft-lease-transaction-reserve-finalize-allow")) fail("v287 draft lease finalization must survive a null-first Firebase transaction callback in runtime, stub QA, and RTDB rules QA.");
if (!appJs.includes("OPERATOR_SESSION_STALE_MS_V288") || !appJs.includes("canResumeSameAccountLeaseV288") || !appJs.includes("v288HeartbeatInFlightByPath") || !appJs.includes('reason: "firebase-unavailable"') || !appJs.includes("v288 pending lease cleanup failed") || !appJs.includes("current == null ? knownLease : current") || !appJs.includes("active = active == null ? knownActive : active") || !appJs.includes("current = current == null ? expectedActive : current") || !appJs.includes("currentRecord = currentRecord == null ? knownRecord : currentRecord") || !operatorQa.includes("operation-lease-reliability-v288") || !operatorQa.includes("leaseTransactionCount !== 1") || !operatorQa.includes("pendingClearedAfterFailure") || !operatorQa.includes("releaseSurvivedNullFirst") || !operatorQa.includes("qa-active-release-v288") || !operatorQa.includes("qa-active-cleanup-v288") || !operatorQa.includes("qa-fence-rotation-v288")) fail("v288 operation lease lifecycle must fail closed, recover stale same-account mobile sessions, coalesce concurrent heartbeats, clean failed finalization reservations, and preserve exact lease, active-pointer, stale-cleanup, and fence-rotation CAS behavior through null-first callbacks under QA.");
if (!appJs.includes("runExistingServerValueTransactionV289") || !appJs.includes("currentValue = currentValue == null ? knownValue : currentValue") || !operatorQa.includes("repairBridgeSurvivedNullFirstCallbacks") || !operatorQa.includes("successFinalizeSurvivedNullFirst")) fail("v289 terminal finish, retry, auto-close, rollback, and convergence transactions must survive provisional null callbacks after reading the exact existing server record.");

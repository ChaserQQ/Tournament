...
- Verified 2026-08-26 KST: v281 app release commit is `da524f759814b487f06f6f9e94f0e860f22136ca` (`Fix operator lease authority v281`) on `main`. Pages run `32919619120` completed successfully; the public index, `src/core/build.js?v=281`, and `src/app.js?v=281` return HTTP 200 and expose the v281 single-authority lease guard while retaining RTDB protocol 279.
- Verified 2026-08-26 KST: local reviewed build is v281, label BUILD v281 OPERATION LEASE SINGLE AUTHORITY; config remains 156, CSS assets remain 278, Firebase rules are unchanged, and `npm.cmd run qa:all` passes.
...
- 2026-08-26 deployed v281: authenticated operators now use the short Firebase server lease as the sole mutation authority. Restored/imported six-hour local `operationLock` data is retired instead of falsely blocking the server lease owner, duplicate legacy ownership controls are hidden, and the canonical session panel provides takeover, release, refresh, and recovery actions. Operator QA reproduces a foreign stale local lock while the browser owns the exact server fence and confirms selection, UI, and local persistence across desktop and mobile widths.
